...
Element app::impl::header() {
	...
	if (web_on) {
		bar.push_back(text("  "));
		bar.push_back(ui::pill(th, "web", th.p.foam));
	}
...
Element app::impl::overlay_view() {
	...
	if (ov == overlay::artifacts) return artifacts_view();
	...
	if (ov == overlay::cheatsheet) {
		...
		return ui::overlay(th, "keys",
		                   vbox({row("ctrl-k", "command palette"),
		                         ...
		                         row("f1", "keys · /settings /cost /artifacts"),
		                         row("cmds", "/plan /websearch /roles /snip"),
